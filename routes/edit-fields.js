...
router.put('/edit-effective-date/:policy_id', async (req, res) => {
    ...
    try {
        ...
        if (!effective_date  || !isValidDate(effective_date)) {
            return res.status(400).json({ error: 'Invalid format for effective_date. Use YYYY-MM-DD.' });
        }
...
router.put('/edit-expiration-date/:policy_id', async (req, res) => {
    ...
    try {
        ...
        if (!expiration_date  || !isValidDate(effective_date)) {
            return res.status(400).json({ error: 'Invalid format for expiration_date. Use YYYY-MM-DD.' });
        }
...
router.put('/edit-policy-name/:policy_id', async (req, res) => {
    ...
    try {
        if (!policy_name) {
            return res.status(400).json({ error: 'Please provide a policy_name to update.' });
        }

        const query = `
            UPDATE policy
            SET policy_name = $2, updated_when = NOW()
            WHERE policy_id = $1
            RETURNING *;
        `;
...
router.put('/edit-categories/:policy_id', async (req, res) => {
    ...
    if (!categories || typeof categories !== 'string') {
        return res.status(400).json({ error: 'Please provide a comma-separated categories string.' });
    }
