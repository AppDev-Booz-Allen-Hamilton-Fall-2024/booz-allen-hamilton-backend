...
router.post("/upload-policy-zip", async (req, res) => {
  ...
  const extractedDir =
    "/Users/rohan/booz_allen/booz-allen-hamilton-backend/policies/CT";
  ...
  try {
    ...
    for (const pdfFile of pdfFiles) {
      ...
      const policyName = path.basename(pdfPath);
      console.log("PDF file:", policyName);
      const result = await db.query(
        "INSERT INTO policy (og_file_path, policy_name) VALUES ($1, $2) RETURNING policy_id",
        [pdfPath, policyName]
      );
