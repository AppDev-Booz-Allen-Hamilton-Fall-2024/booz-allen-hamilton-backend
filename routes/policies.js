...
router.get(`/:policyId/children`, async (req, res) => {
  try {
    ...
    const result = await db.query(
      "SELECT policy_name, effective_date, og_file_path, annotations FROM policy WHERE parent_policy_id = $1",
      [policyId]
    );
    console.log(result);
    ...
    const r = await db.query(
      "SELECT policy_name, effective_date, og_file_path, annotations FROM policy WHERE policy_id = $1",
      [policyId]
    );
